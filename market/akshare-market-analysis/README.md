...
DSH 市场插件：通过受控 AKShare sidecar 提供 A 股、港股和美股实时/最近交易日快照、日/周/月历史行情、K 线和有限的描述性技术指标分析。

## 能力

- `akshare_market_snapshot`：A 股/港股全量快照；美股按 ticker（例如 `AAPL`）查询最近日线记录，支持白名单数值过滤、排序和限量。
- `akshare_stock_history`：A 股/港股/美股历史 OHLCV，支持 daily、weekly、monthly；美股支持 none/qfq，A 股和港股支持 none/qfq/hfq。
...
```powershell
dsh plugin --profile web add @p-dsh-market/akshare-market-analysis@0.1.6
```
...
本插件不提供下单、持仓、目标价或个性化投资建议，不支持分钟线、逐笔、Level-2、期货或任意 AKShare 函数调用。港股和美股数据必须显示数据源可能延迟或仅代表最近交易日。字段漂移、数据不足和上游不可用均 fail closed，不以记忆补全最新行情。
