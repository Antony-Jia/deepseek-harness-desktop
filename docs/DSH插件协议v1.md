...
存储始终先按 `pluginId` 隔离；插件可以使用宿主提供的可选 `sessionId`，在自己的存储范围内进一步区分会话数据。`sessionId` 不属于新的 `scope`，也不改变存储根目录、权限或卸载策略：

- 未提供 `sessionId` 时，数据属于插件级共享数据，适合用户设置、数据库 schema 和跨会话缓存。
- 提供 `sessionId` 时，数据属于该插件的指定会话，适合会话记忆、草稿和临时状态。
- 对于 `directory`，插件不得直接将原始 `sessionId` 拼接到路径；应由宿主校验并解析为插件目录内的安全子目录。
- 对于 `sqlite`，建议共用插件声明的数据库，并通过表中的 `session_id` 字段进行逻辑分区，不建议为每个会话创建独立数据库。
- 删除会话不等于卸载插件，宿主不得仅因会话删除而自动删除持久化数据；相关清理行为应由插件声明或经用户确认。
